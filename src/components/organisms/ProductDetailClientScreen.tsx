...
export function ProductDetailClientScreen() {
  const searchParams = useSearchParams();
  const productId = searchParams.get("productId") ?? "";

  const [products, setProducts] = useState<ProductPageProduct[]>([]);
  const [productDetail, setProductDetail] = useState<ProductPageProduct | null>(
    null,
  );
  ...
  const product = useMemo(() => {
    return productDetail ?? products.find((item) => item._id === productId);
  }, [productDetail, products, productId]);

  const similarProducts = useMemo(() => {
    return products.filter((item) => item._id !== product?._id).slice(0, 8);
  }, [products, product]);
  ...
  if (!product) {
    return (
      <main className="min-h-screen bg-[#f6f6f6] px-5 py-24 text-center">
        <h1 className="text-2xl font-bold text-[#222]">Product not found</h1>
        <p className="mt-3 text-sm text-[#555]">
          Please go back to products and open the product again.
        </p>
      </main>
    );
  }
